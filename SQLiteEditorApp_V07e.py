...
class TableReport:
    """
    Build a simple HTML table from a QTableWidget page and
    send it to either a QPrintPreviewDialog or to an actual printer.
    """
    def __init__(self, qtable: QTableWidget, title: str = "Table Report"):
        self.table = qtable
        self.title = title
...
class DatabaseManager:
    ...
    def connect(self, file_path):
        try:
            ...
            # --- SQLite concurrency tweaks ---------------------------------
            self.conn.execute("PRAGMA journal_mode=WAL")        # persistent, survives restart
            self.conn.execute("PRAGMA synchronous=NORMAL")      # good trade-off for network share
            self.conn.execute("PRAGMA locking_mode=NORMAL")     # shared locks (default)
            self.conn.execute("PRAGMA busy_timeout=8000")        # wait up to 8 s before 'database is locked'
            # ---------------------------------------------------------------
...
# ----------------------
# Progress/Worker Threads for Heavy Operations
# ----------------------
class CsvExportWorker(QThread):
    ...
    def __init__(self, db_manager, table_name, file_path):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            self.db_manager.cursor.execute(f"SELECT COUNT(*) FROM \"{self.table_name}\"")
            total = self.db_manager.cursor.fetchone()[0]
            self.db_manager.cursor.execute(f'SELECT * FROM "{self.table_name}"')
            batch_size = 1000
            written = 0
            with open(self.file_path, 'w', newline='', encoding='utf-8') as f:
                ...
                while True:
                    rows = self.db_manager.cursor.fetchmany(batch_size)
                    if not rows:
                        break
                    writer.writerows(rows)
                    written += len(rows)
                    if total > 0:
                        self.progress.emit(int(written / total * 100))
...
class CsvImportWorker(QThread):
    ...
    def run(self):
        try:
            ...
            ph_list = ", ".join("?" for _ in columns)
            insert_sql = f'INSERT INTO "{self.table_name}" ({col_list}) VALUES ({ph_list})'

            with open(self.file_path, newline='', encoding="utf-8") as fh:
                reader = csv.reader(fh)
                header = next(reader, None)
                if header and header != columns[:len(header)]:
                    self.error.emit("CSV column names do not match table columns")
                    return

                total = sum(1 for _ in open(self.file_path, 'r', encoding='utf-8')) - 1
                fh.seek(0); next(reader, None)
                batch, batch_size, done = [], 1000, 0
                ...
                for row in reader:
                    ...
                    if len(batch) >= batch_size:
                        self.db_manager.cursor.executemany(insert_sql, batch)
                        self.db_manager.conn.commit()        # <<< release writer lock
                        batch.clear()
                        done += batch_size
                        if total > 0:
...
class BatchSaveWorker(QThread):
    ...
    def __init__(self, db_manager, table_name, changes, data_table, columns, pk_col, page_size):
        super().__init__()
        self.db_manager = db_manager
        self.table_name = table_name
    ...
    def run(self):
        try:
            ...
            for i in range(0, total, batch_size):
                ...
                for change in batch:
                    if change[0] == "insert":
                        row_idx = change[1] % self.page_size
                        values = []
                        for col_idx in range(len(self.columns)):
                            item = self.data_table.item(row_idx, col_idx)
                            value = item.text() if item else None
                            values.append(value)
                        if self.pk_col and (values[self.columns.index(self.pk_col)] is None or not values[self.columns.index(self.pk_col)].strip()):
                            self.db_manager.cursor.execute(f'SELECT MAX("{self.pk_col}") FROM "{self.table_name}"')
                            max_id = self.db_manager.cursor.fetchone()[0]
                            values[self.columns.index(self.pk_col)] = str((max_id or 0) + 1)
                        column_names = ", ".join(f'"{c}"' for c in self.columns)
                        ...
                        row_idx, col_idx, value = change[1:]
                        row_id = self.data_table.item(row_idx % self.page_size, 0).text() if self.data_table.item(row_idx % self.page_size, 0) else None
                        if row_id:
                            query = f'UPDATE "{self.table_name}" SET "{self.columns[col_idx]}" = ? WHERE "{self.columns[0]}" = ?'
                            self.db_manager.cursor.execute(query, (value, row_id))
                self.db_manager.conn.commit()        # <<< release writer lock
                self.progress.emit(int((i + batch_size) / total * 100))
            self.db_manager.conn.commit()
            self.finished.emit("success")
        except sqlite3.IntegrityError as e:
            self.error.emit(f"Integrity error: {str(e)}")
            self.db_manager.conn.rollback()
        except sqlite3.Error as e:
            self.error.emit(str(e))
            self.db_manager.conn.rollback()
...
# ----------------------
# SchemaEditor (FULL, with clear on database close)
# ----------------------
class SchemaEditor(QWidget):
    ...
    def load_table(self, table_name):
        ...
        try:
            ...
            self.original_fields = self.fields.copy()  # Reset original fields after loading
    ...
    def update_field_properties(self):
        ...
        self.update_fk_check_state()  # Ensure FK status is re-evaluated
        logging.debug(f"Updated properties for field {field_data['name']}")
    ...
    def apply_changes(self):
        ...
        try:
            ...
            self.refresh_tables()
            self.original_fields = self.fields.copy()  # Reset original fields after successful apply
...
# ----------------------
# DataBrowser (full, with grid line clear on database close)
# ----------------------
class DataBrowser(QWidget):
    ...
    # ---------- report ----------
    def _run_report(self, preview: bool = True):
        if self.table_combo.currentText() == "Select Table":
            QMessageBox.information(self, "Report", "No table selected.")
            return
        rep = TableReport(
            self.data_table,
            title=f"{self.table_combo.currentText()} – Page {self.current_page}"
        )
        try:
            if preview:
                rep.preview(self)
    ...
    def setup_filters_and_update(self, table_name, columns):
        try:
            ...
            while self.filter_layout.count() < len(columns):
                ...
                filter_edit.setPlaceholderText(columns[self.filter_layout.count()])  # Add placeholder with field name
                filter_edit.setStyleSheet("color: gray;")  # Set gray color for placeholder
    ...
    def add_row(self):
        ...
        if pk_col and pk_col in columns:
            ...
            self.data_table.setItem(row_count, pk_idx, item)
        self.changes.append(("insert", row_count + (self.current_page - 1) * self.page_size))
    ...
    def remove_row(self):
        ...
        pk_value = self.data_table.item(row_idx, 0).text() if self.data_table.item(row_idx, 0) else ""
        if pk_value:
            try:
                self.db_manager.cursor.execute(f'DELETE FROM "{table_name}" WHERE "{self.data_table.horizontalHeaderItem(0).text()}" = ?', (pk_value,))
                self.db_manager.conn.commit()
    ...
    def cell_changed(self, row, col):
        ...
        if table_name != "Select Table" and row >= 0:
            ...
            self.changes.append(("update", global_row_idx, col, value))
            logging.info(f"Cell changed at row {global_row_idx}, col {col} for {table_name}")
    ...
    def save_changes(self):
        ...
        try:
            ...
            for change in self.changes:
                if change[0] == "insert":
                    ...
                    for col_idx in range(len(columns)):
                        ...
                        value = item.text() if item else None
                        values.append(value)
                    if pk_col and (values[columns.index(pk_col)] is None or not values[columns.index(pk_col)].strip()):
                        self.db_manager.cursor.execute(f'SELECT MAX("{pk_col}") FROM "{table_name}"')
                        max_id = self.db_manager.cursor.fetchone()[0]
                    ...
                    row_id = self.data_table.item(row_idx % self.page_size, 0).text() if self.data_table.item(row_idx % self.page_size, 0) else None
                    if row_id:
                        query = f'UPDATE "{table_name}" SET "{columns[col_idx]}" = ? WHERE "{columns[0]}" = ?'
                        self.db_manager.cursor.execute(query, (value, row_id))
                elif change[0] == "delete":
                    pass  # Already handled in remove_row
            self.db_manager.conn.commit()
            self.changes.clear()  # Clear changes after successful save
            ...
            QMessageBox.critical(self, "Error", f"Failed to save changes: {str(e)}. Ensure foreign key values are valid.")
            self.db_manager.conn.rollback()
        except sqlite3.Error as e:
...
# ----------------------
# ImportExportTab (full, threaded CSV import/export, progress)
# ----------------------
            ...
            try:
                ...
                with open(file_path, 'r', encoding='utf-8-sig') as f:  # Use utf-8-sig to handle BOM
                    ...
                    if header:
                        ...
                        logging.debug(f"Table columns: {columns[:len(header)]}")
...
class SQLiteEditorApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("SQLite Editor v0.7e")
...
class QueryEditor(QWidget):
    ...
    def refresh_tables(self):
        pass  # No table refresh needed for this tab
