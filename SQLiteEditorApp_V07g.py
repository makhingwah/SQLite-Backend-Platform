...
from PyQt6.QtGui import QTextDocument  # Added for explicit import
import sqlite3
import csv
import os
import logging
...
class DataBrowser(QWidget):
        ...
        try:
            ...
            for row_idx in sorted({i.row() for i in selected_rows}, reverse=True):
                ...
                if rowid is not None:
                    # Delete from database if rowid exists
                    ...
                    # Handle uncommitted rows (newly added but not saved)
            ...
            self.db_manager.conn.commit()  # Commit database changes
            self.update_pagination(table_name)  # Refresh the page to reflect changes
        except sqlite3.Error as e:
        ...
        try:
            ...
            for i, change in enumerate(self.changes):
                ...
                if change[0] == "insert":
                    ...
                    # Assign a new ID if the primary key is empty and auto-increment is implied
                    ...
                    if rowid and 0 <= col_idx < len(columns):
                    ...
                    _, rowid = change
                    if rowid is not None:  # Only delete from DB if rowid exists
                        self.db_manager.cursor.execute(f'DELETE FROM "{table_name}" WHERE rowid = ?', (rowid,))
            ...
            QMessageBox.critical(self, "Error", f"Failed to save changes: {str(e)}. Ensure {pk_col if pk_col else 'primary key'} values are unique.")
            self.db_manager.conn.rollback()
            progress.close()
    ...
    def truncate_all(self):
        ...
        reply = QMessageBox.warning(self, "Confirm Truncate",
                                   f"Are you sure you want to truncate all data in {table_name}? This action cannot be undone.",
                                   QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                self.total_rows = 0
                self.current_page = 1
                self.page_label.setText(f"Page 1 of 1")
                self.prev_btn.setEnabled(False)
                self.next_btn.setEnabled(False)
                QMessageBox.information(self, "Success", f"All data in {table_name} truncated")
                logging.info(f"Truncated all data in {table_name}")
            except sqlite3.Error as e:
                logging.error(f"Failed to truncate table: {str(e)}")
                QMessageBox.critical(self, "Error", f"Failed to truncate table: {str(e)}")

# ----------------------
# SQLiteEditor (Main Window)
# ----------------------
class SQLiteEditor(QMainWindow):
    """Main window assembling all tabs."""
    def __init__(self):
        super().__init__()
        self.db_manager = DatabaseManager.get_instance()
        self.setWindowTitle("SQLite Editor v07g")
    ...
    def _setup_ui(self):
        ...
        self.tabs.addTab(self.browser_tab, "Data")
        self.tabs.addTab(self.imp_tab, "Import/Export")
        self.tabs.addTab(self.query_tab, "SQL")
    ...
    def create_db(self, checked=False) -> bool:
        ...
        if fp:
            ...
            if self.db_manager.connect(fp):  # Use connect instead of create_database for simplicity
    ...
    def close_db(self, checked=False) -> bool:
        ...
        # Stop the refresh timer to prevent callbacks during cleanup
        ...
        # Clear tab content but retain tab structure
        if self.schema_tab:
    ...
    def closeEvent(self, ev):
        ...
        if self._ensure_discard_changes():
            self._refresh_timer.stop()  # Ensure timer is stopped
